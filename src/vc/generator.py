...
class Generator:
    ...
    def generate(self, contract_name: str, function_name: str, loop_index: int, inv: str) -> tuple[str, str, str]:
        ...
        base = [smt_declarations, smt_inv_fun_definition]
        entry_args = ' '.join(entry_ssa_map[v] for v in loop_vars if v in entry_ssa_map)
        exit_args = ' '.join(exit_ssa_map[v] for v in loop_vars if v in exit_ssa_map)

        pre_vc = '\n'.join(base + [
            smt_pre_fun_definition,
            f'(assert (not (=> pre-f (inv-f {entry_args}))))',
            '(check-sat)'
        ]) 
        trans_vc = '\n'.join(base + [
            smt_trans_fun_definition,
            f'(assert (not (=> (and (inv-f {entry_args}) trans-f) (inv-f {exit_args}))))',
            '(check-sat)'
        ])
        post_vc = '\n'.join(base + [
            smt_post_fun_definition,
            f'(assert (not (=> (inv-f {exit_args}) post-f)))',
            '(check-sat)'
        ])
        ...
        for node in trans_path:
            for ir in node.irs_ssa:
                if isinstance(ir, OperationWithLValue) and ir.lvalue:
                    ...
                    if isinstance(ir, Phi) and ir.rvalues:
                        entry_var = sorted(ir.rvalues, key=lambda v: self._var_to_smt(v))[0]
                        entry_ssa_map[base_name] = self._var_to_smt(entry_var)
                    if hasattr(ir, 'read') and ir.read:
                        for var in ir.read:
                            if self._get_base_name(var) == base_name:
                                entry_ssa_map.setdefault(base_name, self._var_to_smt(var))
